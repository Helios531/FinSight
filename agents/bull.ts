...
const queries = [
  "revenue growth operating improvement customer demand guidance margin expansion",
  "positive cash flow profitability backlog retention market share management optimism",
  "earnings call prepared remarks strengths growth drivers improving metrics"
];
...
export async function runBullAgent(context: AgentContext): Promise<AgentResult> {
  ...
  const { value, tokenUsage } = await completeJson<LlmClaims>({
    trace: { agent: "Bull Agent" },
    schema: llmClaimsSchema,
    system:
      "You are the Bull Agent for institutional financial document analysis. Argue only bullish evidence. Do not invent facts. Every claim must cite provided evidence ids. Avoid generic finance adjectives.",
    user: JSON.stringify({
    ...
    fallback: () => ({
      claims: [
        {
          title: "Positive operating signal",
          claim: fallbackClaim("positive", evidence[0]?.chunk.text),
          citationIds: evidence.slice(0, 2).map((item) => item.chunk.id),
          confidence: Math.max(0.45, meanScore(evidence.slice(0, 2)))
        }
      ]
    })
  });
